...
def load_and_extract_features_parallel(folder, class_label):
    feature_list = []
    labels = []
    ...
    # Use parallel processing for image loading and feature extraction
    results = Parallel(n_jobs=-1)(delayed(process_image)(filename) for filename in os.listdir(folder))
...
# 4. 加载并提取特征
features_PAH, labels_PAH = load_and_extract_features_parallel(PAH_folder, class_label_PAH)
features_noPAH, labels_noPAH = load_and_extract_features_parallel(noPAH_folder, class_label_noPAH)

test_features_PAH, test_labels_PAH = load_and_extract_features_parallel(test_PAH_folder, class_label_PAH)
test_features_noPAH, test_labels_noPAH = load_and_extract_features_parallel(test_noPAH_folder, class_label_noPAH)
...
for train_index, test_index in kf.split(X, Y):
    ...
    test_X_for_val_scaled = scaler.transform(test_X)
    # 7. 训练分类器
    mlp_classifier.fit(X_train_scaled, y_train)
    # 8. 预测
    y_pred = mlp_classifier.predict(X_test_scaled)
    y_pred_test = mlp_classifier.predict(test_X_for_val_scaled)
    # 如果模型支持预测概率
